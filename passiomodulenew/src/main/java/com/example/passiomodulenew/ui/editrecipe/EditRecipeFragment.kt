...
import com.example.passiomodulenew.ui.edit.EditFoodFragment
import com.example.passiomodulenew.ui.edit.IngredientAdapter
...
import com.passio.passiomodulenew.databinding.FragmentEditRecipeBinding
...
class EditRecipeFragment : BaseFragment<EditRecipesViewModel>() {
            ...
            servingQuantity.setOnEditorActionListener { v, actionId, event ->
                if (actionId == EditorInfo.IME_ACTION_DONE) {
                    try {
                        ...
                        viewModel.updateServingQuantity(quantity, EditFoodFragment.UpdateOrigin.QUANTITY)
    ...
    private fun updateFoodRecord(foodRecord: FoodRecord, origin: EditFoodFragment.UpdateOrigin) {
//        renderNutrients(foodRecord)
        binding.delete.isVisible = viewModel.isEditRecipe()
        renderServingSize(foodRecord, origin)
        if (origin == EditFoodFragment.UpdateOrigin.INGREDIENT) {
...
//            renderIngredients(foodRecord)
    ...
    private fun renderServingSize(foodRecord: FoodRecord, origin: EditFoodFragment.UpdateOrigin? = null) {
        ...
        with(binding) {
            ...
            if (origin != EditFoodFragment.UpdateOrigin.QUANTITY) {
            ...
            if (origin != EditFoodFragment.UpdateOrigin.SEEKBAR) {
...
//            }
    ...
    private val servingUnitListener = object : AdapterView.OnItemSelectedListener {
        override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
            viewModel.updateServingUnit(position, EditFoodFragment.UpdateOrigin.UNIT)
        }
        ...
        override fun onSeeking(seekParams: SeekParams) {
            ...
            if (value % 0.5 == 0.0) {
                viewModel.updateServingQuantity(
                    ...
                    EditFoodFragment.UpdateOrigin.SEEKBAR
                )
            } else {
                val multiplier = (value / 0.5).toInt()
                val roundedValue = 0.5 * multiplier
                viewModel.updateServingQuantity(roundedValue, EditFoodFragment.UpdateOrigin.SEEKBAR)
            }
        }
